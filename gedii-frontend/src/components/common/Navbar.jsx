...
export default function Navbar() {
  ...
  return (
    <header style={styles.navbar} className={scrolled ? 'navbar-scrolled' : ''}>
        ...
        <div style={styles.right} ref={menuRef}>
          ...
          {menuOpen && (
            <div style={styles.dropdown} className="page-transition">
              <button onClick={goToProfil} style={styles.dropdownItem}>
                <UserIcon size={16} />
                Mon profil
              </button>
              <button onClick={logout} style={{ ...styles.dropdownItem, color: 'var(--color-accent-red)' }}>
                <LogOut size={16} />
                Déconnexion
              </button>
            </div>
...
const styles = {
  ...
  dropdown: {
    ...
    borderRadius: 'var(--radius)', boxShadow: '0 8px 24px var(--color-shadow)',
    minWidth: '180px', overflow: 'hidden',
  },
  dropdownItem: {
    display: 'flex', alignItems: 'center', gap: '8px', width: '100%',
    padding: '10px 14px', background: 'transparent', border: 'none',
    fontSize: '14px', color: 'var(--color-text)', textAlign: 'left', cursor: 'pointer',
  },
};